...
if nargin<3
    ...
    if nargin<2
        options = struct();
        if nargin<1
            P_e = 250e6; % [W] Puissance energetique de l'installation
        end
    end
end
...
if isfield(options,'reheat')
    reheat = options.nsout;
else
    reheat = 0;  
end

if isfield(options,'T_max')
    T_max = 273.15 + options.T_max;
else
    T_max = 273.15+525;  % [K]
end

if isfield(options,'T_ext')
    T_ext = 273.15 + options.T_ext;
else
    T_ext = 273.15+15;  % [K]
end

if isfield(options,'p3_hp')
    p3_hp = 10e5 * options.p3_hp;
else
    p3_hp = 10e5 * 200;  % [Pa]
end

if isfield(options,'TpinchRiv')
    TpinchRiv = options.TpinchRiv;
else
    TpinchRiv = 10;  % [K]
end
